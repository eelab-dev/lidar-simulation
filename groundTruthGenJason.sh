...
if jq -e '.groundTruth_generation | length > 0' "$config" > /dev/null 2>&1; then
        ...
        if [  -f "$input_model_file_path" ]; then
            ...
            pixelizationData_dir=$(jq -r '.groundTruth_generation.output_rawData_dir // "pixelizedData"' "$config")
            pixelizationData_dir="${config_dir}/${pixelizationData_dir}"
            mkdir -p "$pixelizationData_dir"     

            pixelizationData_file="${global_prefix}_pixelized_${i}.h5"
            pixelizationData_file_path="${pixelizationData_dir}/${pixelizationData_file}"
            ...
            if jq -e '.groundTruth_generation.camera_config' "$config" > /dev/null 2>&1; then


                input_camera_dir=$(jq -r '.groundTruth_generation.camera_config.camera_dir // "camera"' "$config")
                input_camera_dir="${config_dir}/${input_camera_dir}"
                # Try to get camera_config_prefix, fallback empty if not found
                camera_config_prefix=$(jq -r '.groundTruth_generation.camera_config.camera_config_prefix // empty' "$config")
                static_camera_file=$(jq -r '.groundTruth_generation.camera_config.static_camera_config // empty' "$config")
            ...
            if jq -e '.groundTruth_generation.ssp' "$config" > /dev/null 2>&1; then
                ssp=$(jq -r '.groundTruth_generation.ssp' "$config")
                simulation_flag+=" --ssp ${ssp}"
            fi

            if jq -e '.groundTruth_generation.width' "$config" > /dev/null 2>&1; then
                width=$(jq -r '.groundTruth_generation.width' "$config")
                simulation_flag+=" --width ${width}"
            fi

            if jq -e '.groundTruth_generation.height' "$config" > /dev/null 2>&1; then
                height=$(jq -r '.groundTruth_generation.height' "$config")
            ...
            ./syclImplementation/build/LiDARGroundtruth\
            --model "${input_model_file_path}" \
            --output "${pixelizationData_file_path}" \
            --seed 4 \
            $simulation_flag || { 
    ...
    if jq -e '.groundTruthImage | length > 0' "$config" > /dev/null 2>&1; then
        input_pixelized_dir=$(jq -r '.groundTruthImage.input_pixelized_dir' "$config")
        ...
        if [ -f "$pixelized_file_path" ]; then
            depthImage_dir=$(jq -r '.groundTruthImage.output_depthImage_dir // "depthImage"' "$config")
            ...
            depthImage_file="${global_prefix}_GroundTruthDepth${i}.png"
            depthImage_file_path="${depthImage_dir}/${depthImage_file}"
            simulation_flag+=" --output_image ${depthImage_file_path}"

            if jq -e '.groundTruthImage.bin_number' "$config" > /dev/null 2>&1; then
                bin_number=$(jq -r '.groundTruthImage.bin_number' "$config")
                simulation_flag+=" --bin_number ${bin_number}"
            fi

            if jq -e '.groundTruthImage.min_range' "$config" > /dev/null 2>&1; then
                min_range=$(jq -r '.groundTruthImage.min_range' "$config")
                simulation_flag+=" --min_range ${min_range}"
            fi

            if jq -e '.groundTruthImage.bin_width' "$config" > /dev/null 2>&1; then
                bin_width=$(jq -r '.groundTruthImage.bin_width' "$config")
                simulation_flag+=" --bin_width ${bin_width}"
            fi

            echo $simulation_flag
            python3 pythonScripts/groundTruthImage.py $simulation_flag || {
